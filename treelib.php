...
function tree_delete_rec($id){
	...
	if ($istree){
    	$sql = "
    	    SELECT 
    	        id
    	    FROM 
    	        {$CFG->prefix}{$table}cognitivefactory_operatordata
    	    WHERE
	...
	delete_records('cognitivefactory_operatordata', 'id', $id); 
	$deleted[] = $id;
	return $deleted;
}
...
function cognitivefactory_tree_updateordering($cognitivefactoryid, $groupid, $userid, $id, $istree){
	...
	$res =  get_record('cognitivefactory_operatordata', 'id', $id);
	if (!$res) return;
	...
	if( $nextsubs = get_record_sql($sql)) {
		...
		foreach($nextsubs as $asub){
			...
			update_record('cognitivefactory_operatordata', $objet);
			$ordering++;
		}
	}
}
...
function cognitivefactory_tree_left($cognitivefactoryid, $userid, $groupid, $id){
	...
	if($ress = get_records_sql($sql)){
		foreach($ress as $res){
			...
			update_record('cognitivefactory_operatordata AS od', $objet);
