...
print_heading("<img src=\"{$CFG->wwwroot}/mod/cognitivefactory/operators/{$page}/pix/enabled_small.gif\" align=\"left\" width=\"40\" /> " . get_string('tree', 'cognitivefactory'));
?>
<center>
<?php
...
<?php
        hierarchize_print_level($cognitivefactory->id, $cm, null, $currentgroup, false, $child->odid, $i+1, $indent, $current_operator->configdata);
        $i++;
...
?>
    <tr>
        <td colspan="2">
            <br/><input type="button" name="clear_btn" value="<?php print_string('clearalldata', 'cognitivefactory') ?>" onclick="document.forms['treeform'].what.value='clearall';document.forms['treeform'].submit();" />
        </td>
    </tr>
<?php
}
    ...
    print_string('notreeset', 'cognitivefactory');
?>
   
    <p><center><input type="submit" name="go_btn" value="<?php print_string('maketree', 'cognitivefactory') ?>" /></center></p>
<?php
