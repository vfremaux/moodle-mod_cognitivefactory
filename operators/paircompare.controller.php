...
if ($action == 'startpaircompare'){
    ...
    if (!delete_records('cognitivefactory_operatordata', 'cognitivefactoryid', $cognitivefactory->id, 'userid', $USER->id, 'operatorid', $page)){
    ...
    if (!insert_record('cognitivefactory_operatordata', $record)){
        error("Could not insert marking record");
    }

    print_heading(get_string('paircompare','cognitivefactory'));
    include "{$CFG->dirroot}/mod/cognitivefactory/operators/order/paircompare.html";
    return -1;
}
...
if ($action == 'nextpaircompare'){
    ...
    $select = "
       ...
       userid = {$USER->id}
    ";
    $orderrecord1 = get_record_select('cognitivefactory_operatordata', $select);
    if (!$orderrecord1){
        ...
        if (!insert_record('cognitivefactory_operatordata', $orderrecord1)){
        ...
        if ($choice == $response1){
            ...
            if (!update_record('cognitivefactory_operatordata', $orderrecord1)){
    ...
    $select = "
       ...
       userid = {$USER->id}
    ";
    $orderrecord2 = get_record_select('cognitivefactory_operatordata', $select);
    if (!$orderrecord2){
        $orderrecord2 = &$orderrecord1;
        $orderrecord2->itemsource = $response2;
        $orderrecord2->intvalue = ($choice == $response2) ? 1 : 0 ;
        if (!insert_record('cognitivefactory_operatordata', $orderrecord2)){
            error("Could not insert record");
        }
    }
    else{
        if ($choice == $response2){
            ...
            if (!update_record('cognitivefactory_operatordata', $orderrecord2)){
                error("Could not update record");
            }
        }
    }
    ...
    $select = "
       ...
       userid = {$USER->id}
    ";
    if (!$marking = get_record_select('cognitivefactory_operatordata', $select)){
    ...
    if (empty($markarray)){
        ...
        if (!update_record('cognitivefactory_operatordata', $marking)){
