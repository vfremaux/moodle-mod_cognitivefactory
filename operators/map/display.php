...
if (count($responses) > $MAP_MAX_DATA){
    notice(get_string('toomuchdata', 'cognitivefactory', $MAP_MAX_DATA));
    return;
}
...
print_heading(get_string('mapofresponses', 'cognitivefactory'));
...
if ($responses){
            ...
            if (@$current_operator->configdata->quantified && $current_operator->configdata->quantifiertype == 'multiple'){
                ...
                if (@$current_operator->configdata->quantified){
                    ...
                    if (@$map[$responserow->id][$responsecol->id]){
                        $mapitem = "<img src=\"{$CFG->wwwroot}/mod/cognitivefactory/operators/map/pix/check.gif\"/>";
                    }
                }
            }
    ...
    print_string('noresponses', 'cognitivefactory');
}
...
<?php 
    if (empty($order)){
?>
            <input type="submit" name="checkcycles_btn" value="<?php print_string('checkcycles','cognitivefactory') ?>" />
            &nbsp;<?php print_string('showmatrixproduct', 'cognitivefactory') ?>: <input type="checkbox" name="cycleop" value=">" />
<?php
    } else {
        if ($order > 1){
?>
            <input type="button" name="orderminus_btn" value="<?php print_string('orderminusone','cognitivefactory', $order - 1) ?>" onclick="document.forms['orderform'].order.value = <?php echo $order - 1; ?>;document.forms['orderform'].submit();" />
<?php
        }
?>
            &nbsp;<input type="button" name="orderplus_btn" value="<?php print_string('orderplusone','cognitivefactory', $order + 1) ?>" onclick="document.forms['orderform'].order.value = <?php echo $order + 1; ?>;document.forms['orderform'].submit();" />
            &nbsp;<input type="button" name="display_btn" value="<?php print_string('displaynormal','cognitivefactory') ?>" onclick="document.forms['orderform'].what.value = '';document.forms['orderform'].submit();" />
<?php
    }
?>
        </td>
