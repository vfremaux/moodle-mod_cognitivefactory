                ...
                switch($current_operator->configdata->quantifiertype){
                    case 'multiple':
                        ...
                        if (!empty($itemdata)){
                            ...
                            $maplink = "<a href=\"view.php?id={$cm->id}&amp;operator=map&amp;what=inputmultiple&amp;source={$responserow->id}&amp;dest={$responsecol->id}\">".get_string('inputdata', 'cognitivefactory').'</a>';
                        }
...
?>
    <tr>
        <td colspan="<?php echo count($responses) + 1; ?>">
            <br/><input type="submit" name="go_btn" value="<?php print_string('saveconnections', 'cognitivefactory') ?>" />
            &nbsp;<input type="button" name="clear_btn" value="<?php print_string('clearconnections', 'cognitivefactory') ?>" onclick="document.forms['mapform'].what.value='clearmappings';document.forms['mapform'].submit();" />
        </td>
    </tr>
</table>
</form>
