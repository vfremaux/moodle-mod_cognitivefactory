...
<p><table align="center" width="80%">
    <tr>
        <td colspan="<?php echo $cognitivefactory->numcolumns * 2 ?>">
            <?php print_heading(get_string('myresponses', 'cognitivefactory')) ?>
        </td>
    </tr>
    <tr valign="top">
<?php
    cognitivefactory_print_responses_cols($cognitivefactory, $myresponses, false, has_capability('mod/cognitivefactory:manage', $context));
?>
    </tr>
</table></p>
<?php
    if (!$cognitivefactory->privacy){
?>
<p>
<table align="center" width="80%">
    <tr>
        <td colspan="<?php echo $cognitivefactory->numcolumns ?>">
            <?php print_heading(get_string('otherresponses', 'cognitivefactory')) ?>
        </td>
    </tr>
    <tr>
<?php
    ...
    foreach ($otherresponses as $response){
        $deletecheckbox = (has_capability('mod/cognitivefactory:manage', $context)) ? "<input type=\"checkbox\" name=\"items[]\" value=\"{$response->id}\" /> " : '' ;
...
<?php
    if (has_capability('mod/cognitivefactory:manage', $context)) echo '</form>';
    }
}
else{
...
if ((($cognitivefactory->flowmode == 'parallel' || $cognitivefactory->phase == PHASE_COLLECT) and
     (($cognitivefactory->numresponses > count($myresponses)) || $cognitivefactory->numresponses == 0)) or has_capability('mod/cognitivefactory:manage', $context)){
    ...
    if (!$groupmode or groups_is_member($currentgroup) or has_capability('mod/cognitivefactory:manage', $context)){
?>
    <td>
