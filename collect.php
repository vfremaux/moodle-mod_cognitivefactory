...
/// Just display responses, sorted in alphabetical order
	...
	if (!empty($cognitivefactory->collectrequirement)) echo $OUTPUT->box($cognitivefactory->collectrequirement);
	...
	*/
	if ((($cognitivefactory->flowmode == 'parallel' || $cognitivefactory->phase == PHASE_COLLECT) and
	    ...
	    if (has_capability('mod/cognitivefactory:import', $context)){
	    	...
	    	echo $OUTPUT->single_button(new moodle_url($CFG->wwwroot.'/mod/cognitivefactory/import.php', $options), get_string('importideas','cognitivefactory'));
	    	echo '</td>';
	    }
	    echo '</tr></table>';
	}
